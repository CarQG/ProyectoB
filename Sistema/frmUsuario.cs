...
namespace Sistema
{
    public partial class frmUsuario : Form
    {
        ...
        private void frmUsuario_Load(object sender, EventArgs e)
        {
            ...
            dgdata.Columns.Add("Usuario", "Usuario");
            dgdata.Columns.Add("Nombres", "Nombres");
            dgdata.Columns.Add("Correo", "Correo");
            dgdata.Columns.Add("Clave", "Clave");
            dgdata.Columns.Add("Permiso", "Permiso");
            

            dgdata.Columns["Usuario"].Width = 100;
        ...
        private void btnnuevo_Click(object sender, EventArgs e)
        {
            using (var form = new mdUsuario(true,null))
            {
                ...
                if (result == DialogResult.OK)
                {
                    ...
                    row.Cells["Usuario"].Value = obj.Usuario;
                    row.Cells["Nombres"].Value = obj.Nombres;
                    row.Cells["Correo"].Value = obj.Correo;
                    row.Cells["Clave"].Value = obj.Contraseña;
                    row.Cells["Permiso"].Value = obj.TipoPermiso;
                }
            }
        }
        ...
        private void mostrar_personas() {
            ...
            string mensaje;
            List<Persona> olista = PersonaLogica.Instancia.Listar(out mensaje).Where(p => p.TipoPermiso != "CLIENTE").ToList();
            ...
            if (string.IsNullOrEmpty(mensaje))
            {
                foreach (Persona op in olista)
                {
                    ...
                    row.Cells["Id"].Value = op.IdUsuario;
                    row.Cells["Usuario"].Value = op.Usuario;
                    row.Cells["Nombres"].Value = op.Nombres;
                    row.Cells["Correo"].Value = op.Correo;
                    row.Cells["Clave"].Value = op.Contraseña;
                    row.Cells["Permiso"].Value = op.TipoPermiso;
                }
            ...
            if (dgdata.Columns[e.ColumnIndex].Name == "btnEliminar")
            {
                ...
                if (index >= 0)
                {
                    if (MessageBox.Show("¿Desea eliminar el usuario?", "Mensaje", MessageBoxButtons.YesNo, MessageBoxIcon.Question) == DialogResult.Yes)
                    {
                        ...
                        int respuesta = PersonaLogica.Instancia.Eliminar(new Persona() {IdUsuario = _idpersona },out mensaje);
                        if (respuesta > 0)
                        {
                ...
                if (index >= 0)
                {
                    ...
                    {
                        IdUsuario = int.Parse(dgdata.Rows[index].Cells["Id"].Value.ToString()),
                        Usuario = dgdata.Rows[index].Cells["Usuario"].Value.ToString(),
                        Nombres = dgdata.Rows[index].Cells["Nombres"].Value.ToString(),
                        Correo = dgdata.Rows[index].Cells["Correo"].Value.ToString(),
                        Contraseña = dgdata.Rows[index].Cells["Clave"].Value.ToString(),
                        TipoPermiso = dgdata.Rows[index].Cells["TipoPersona"].Value.ToString()
                    };
                    ...
                    using (var form = new mdUsuario(false, opersona))
                    {
                        ...
                        if (result == DialogResult.OK)
                        {
                            ...
                            if (objpersona.IdUsuario == obj.IdUsuario)
                            {
                                ...
                                dgdata.Rows[index].Cells["Id"].Value = obj.IdUsuario;
                                dgdata.Rows[index].Cells["Usuario"].Value = obj.Usuario;
                                dgdata.Rows[index].Cells["Nombres"].Value = obj.Nombres;
                                dgdata.Rows[index].Cells["Correo"].Value = obj.Correo;
                                dgdata.Rows[index].Cells["Clave"].Value = obj.Contraseña;
                                dgdata.Rows[index].Cells["Permiso"].Value = obj.TipoPermiso;
                            }
