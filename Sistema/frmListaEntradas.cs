...
namespace Sistema
{
    public partial class frmListaEntradas : Form
    {
        ...
        private void frmServicioIngresos_Load(object sender, EventArgs e)
        {
            ...
            dgdata.Columns["cantidad"].Width = 500;
        ...
        private void btnnuevo_Click(object sender, EventArgs e)
        {
            ...
            using (var form = new mdServicioIngreso(true, null,"LICORES"))
            {
        ...
        private void mostrar_servicios()
        {
            ...
            string mensaje;
            List<Servicio> olista = ServicioLogica.Instancia.Listar(out mensaje).Where(s => s.TipoServicio == "LICORES").ToList();
            ...
            else if (dgdata.Columns[e.ColumnIndex].Name == "btnEditar")
            {
                if (index >= 0)
                {
                    Servicio oservicio = new Servicio()
                    {
                        ...
                        TipoServicio = "LICORES"
                    };

                    using (var form = new mdServicioIngreso(false, oservicio, "LICORES"))
                    {
                        var result = form.ShowDialog();
                        if (result == DialogResult.OK)
                        {
