...
namespace Sistema
{
    public partial class frmListaSalidas : Form
    {
        ...
        private void frmServicioEgresos_Load(object sender, EventArgs e)
        {
        ...
        {
            using (var form = new mdServicioIngreso(true, null, "CERVEZAS"))
            {
                ...
                if (result == DialogResult.OK)
                {
                    ...
                    row.Cells["Id"].Value = obj.Id_L;
                    row.Cells["Descripcion"].Value = obj.Descripcion;
                    row.Cells["Cantidad"].Value = obj.Cantidad;
                }
            }
        }
        ...
        private void mostrar_servicios()
        {
            ...
            string mensaje;
            List<Servicio> olista = ServicioLogica.Instancia.Listar(out mensaje).Where(s => s.TipoServicio == "CERVEZAS").ToList();
            ...
            if (string.IsNullOrEmpty(mensaje))
            {
                foreach (Servicio op in olista)
                {
                    ...
                    row.Cells["Id"].Value = op.Id_L;
                    row.Cells["Descripcion"].Value = op.Descripcion;
                    row.Cells["Cantidad"].Value = op.Cantidad;
                }
            }
            else
            {
        ...
        private void dgdata_CellContentClick(object sender, DataGridViewCellEventArgs e)
        {
            ...
            if (dgdata.Columns[e.ColumnIndex].Name == "btnEliminar")
            {
                ...
                if (index >= 0)
                {
                    if (MessageBox.Show("¿Desea eliminar el servicio?", "Mensaje", MessageBoxButtons.YesNo, MessageBoxIcon.Question) == DialogResult.Yes)
                    {
                        ...
                        int respuesta = ServicioLogica.Instancia.Eliminar(new Servicio() { Id_L = _id }, out mensaje);
                        if (respuesta > 0)
                        {
            ...
            else if (dgdata.Columns[e.ColumnIndex].Name == "btnEditar")
            {
                if (index >= 0)
                {
                    ...
                    {
                        Id_L = int.Parse(dgdata.Rows[index].Cells["Id"].Value.ToString()),
                        Descripcion = dgdata.Rows[index].Cells["Descripcion"].Value.ToString(),
                        Cantidad = dgdata.Rows[index].Cells["Cantidad"].Value.ToString(),
                        TipoServicio = "CERVEZAS"
                    };

                    using (var form = new mdServicioIngreso(false, oservicio, "CERVEZAS"))
                    {
                        var result = form.ShowDialog();
                        if (result == DialogResult.OK)
                        {
                            Servicio obj = form.oServicio;
                            dgdata.Rows[index].Cells["Id"].Value = obj.Id_L;
                            dgdata.Rows[index].Cells["Descripcion"].Value = obj.Descripcion;
                            dgdata.Rows[index].Cells["Cantidad"].Value = obj.Cantidad;
